...
std::string
NewsManager::downloadImage(const std::string &image_url_to_download) {
...
void NewsManager::parseRssItemThread(ParseItemData &itemData) {
  ...
  if (itemData.imageUrl.empty()) {
    for (int i = 0; i < MAX_RETRIES; ++i) {
      ...
      if (!og_image_url.empty()) {
        qDebug() << "Found Open Graph image URL on attempt" << i + 1
                 << "for link:" << itemData.link;
        break;
      }
      qDebug() << "Attempt" << i + 1
               << ": No Open Graph image URL found for link:" << itemData.link
               << ". Retrying...";
    }
  } else {
  ...
  if (!og_image_url.empty()) {
    for (int i = 0; i < MAX_RETRIES; ++i) {
      ...
      if (!itemData.imageUrl.empty()) {
        qDebug() << "Downloaded image on attempt" << i + 1
                 << "for link:" << itemData.link;
        break;
      }
      qDebug() << "Attempt" << i + 1
               << ": No image downloaded for link:" << itemData.link
               << ". Retrying...";
    }
  } else {
    itemData.imageUrl = "";
